...
export function publicUrl(path: string): string {
  ...
  // equal to "/my-base", then passing the path equal to "manifest.json" will not
  // give us the expected result, it will actually be "/manifest.json", but the expected
  // one is "/my-base/manifest.json". This is due to the URL constructor.
  ...
  return new URL(
    ...
    // equal to "/manifest.json", we will not get the expected result like
    // "/my-base/manifest.json", but "/manifest.json".
    path.replace(/^\/+/, ''),
    isBaseAbsolute
      ? baseUrl
